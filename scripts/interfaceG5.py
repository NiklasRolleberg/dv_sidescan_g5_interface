...
class DVSFileWriter:
    ...
    class V1_FileHeader:
        def __init__(self, range):
            ...
            self.lineRate = 750.0 / ( float(self.nSamples) * self.sampleRes)    #[ ping/s ] (float)
        ...
        if(len(data) == 1004): #New SSS data received
            ...
            if(ping_nr == self.current_ping_number): #We should have received both channels
                if(self.ch0_received and self.ch1_received):
                    ...
                    if(self.file_writer):
                        self.file_writer.write(self.sss_message.starboard_channel, self.sss_message.port_channel)
